...
def is_in(agentip, agentid, agentname):
	...
	for ss in import_search_strings:
		...
		if search_string in agentip:
			...
			temp_container = requests.delete(url, headers=tenable_header)
			newentry = ("%s - %s was removed from the group %s" % (agentip, agentname, search_group_id))		
			newFile = open("tenable_remove_from_group.log", "a")
			newFile.write("%s\n" % (newentry))
			newFile.close()		
			print (newentry)
			time.sleep(.3)		
...
tempFile = open("temp_remove_from_group.json", "w")
tempFile.write(tempFile_data.content)
tempFile.close()


originalFile = "temp_remove_from_group.json"
...
print("%d agents were removed." %(added_count))
print("tenable_removed_from_group.log has been saved to your current directory")
